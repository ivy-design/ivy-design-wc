...
const scrollLeft = () => {
  const left = getComputedStyle(tableInnerEl.value)['left'];
...
const scrollRight = () => {
  const innerStyle = getComputedStyle(tableInnerEl.value);
  const left = innerStyle['left'];
  const width = innerStyle['width'];
  const leftN = parseFloat(left);
  const widthN = parseFloat(width);
  const wrapWidth = parseFloat(getComputedStyle(tabHeaderWrap.value)['width']);
...
onMounted(() => {
  const list = slotEl.value.assignedElements().filter(c => c.nodeName === 'IVY-TAB-PANE');
...
<template>
  <div class="ivy-tab-header">
    <div class="ivy-tab-header-arrow ivy-tab-header-arrow-left" ref="btnLeft" @click="scrollLeft"></div>
      <div class="ivy-tab-wrap" ref="tabHeaderWrap">
        <div class="ivy-tab-wrap-inner" ref="tableInnerEl">
          <div v-for="item in children" :key="item.index" class="ivy-tab-wrap-item" :data-index="item.index" @click="headerItemClick">
            {{ item.label }}
          </div>
          <div class="ivy-tab-wrap-line" :style="{width: `${w}px`, left: `${x}px`}"></div>
        </div>
      </div>
      <div class="ivy-tab-header-arrow ivy-tab-header-arrow-right" ref="btnRight" @click="scrollRight"></div>
  </div>
  <div>
